...
async function writeVapLog(level, category, message, detail, meta = {}) {
  ...
  if (level === "ERROR") {
    ...
    console.info("[Videos Auto Play]", line);
  }

  void persistLogBuffer();
  scheduleLogFileFlush(level === "ERROR");
}
...
void loadLogBuffer().then(() => {
  vapLog.info("lifecycle", "background logger ready");
});
